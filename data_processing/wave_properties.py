...
def phase_difference_div_improved(
    sig1,
    sig2,
    fs,
    n_pi=0,
    ...
    S1f, freq, _ = pp.improved_fft(
        sig1,
        fs=fs,
        interpolation_factor=8,
        methods=["zero_padding", "windowing", "interpolation"],
    )
    S2f, freq, _ = pp.improved_fft(
        sig2,
        fs=fs,
        interpolation_factor=8,
        methods=["zero_padding", "windowing", "interpolation"],
...
def phase_difference_div(
    sig1, sig2, fs=500000, n_pi=0, pos_only=False, freq_min=0, freq_max=30000
):
    S1f = np.fft.fft(sig1)
    S2f = np.fft.fft(sig2)
    if pos_only:
...
def phase_difference(sig1, sig2, method="div", n_pi=0, freq_max=30000):
    if method == "sub":
        return phase_difference_sub(sig1, sig2)
    elif method == "div":
        return phase_difference_div(sig1, sig2, n_pi=n_pi, freq_max=freq_max)
    elif method == "gpt":
...
def phase_difference_plot(
    ...
    phase_diff = phase_difference(
        sig1, sig2, method=method, n_pi=n_pi, freq_max=BANDWIDTH[1]
    )
...
def compress_and_cut_df_touch(
    df, channels=["channel 1", "channel 3"], direct_samples=55
):
    compressed_df = df.copy()
    for ch in channels:
        compressed_df[ch] = compress_single_touch(
            df[ch], set_threshold_man=True, n_sampl=direct_samples
        )
...
def phase_plotting(
    ...
    compressed_df = compress_chirp(
        df_sig_only, chirp, use_recorded_chirp=use_recorded_chirp
    )
    ...
    start_index_ch1 = get_first_index_above_threshold(
        compressed_df[channels[0]], threshold1
    )
    end_index_ch1 = start_index_ch1 + duration_cut
    start_index_ch2 = get_first_index_above_threshold(
        compressed_df[channels[1]], threshold2
    )
...
def theoretical_group_phase_vel(freqs, material="teflon", plot=False, kind="cubic"):
    ...
    (
        ...
        material,
    ) = theoretical_velocities(freqs, material=material)
    ...
    v_g_numpy = corrected_phase_velocities / (
        1 - (d_v_ph * omega / corrected_phase_velocities)
    )
...
def theoretical_velocities(freq, material="teflon"):
    ...
    if material == "teflon":
        ...
        density = 935.65
        poisson_ratio = 0.425408
        youngs_modulus = 2.19921e9
        plate_thickness = 0.02
        correction_factor = get_estimated_gamma_correction(poisson_ratio)
    ...
    # density = 950  # kg/m^3
    # poisson_ratio = 0.42  # -
    phase_velocity_longitudinal = np.sqrt(
        youngs_modulus / (density * (1 - poisson_ratio**2))
    )
    phase_velocity_shear = phase_velocity_longitudinal * np.sqrt(
        (1 - poisson_ratio) / 2
    )
    phase_velocities_flexural = np.sqrt(
        1.8 * phase_velocity_longitudinal * plate_thickness * freq
    )
...
def plot_theoretical_velocities(freq, material="teflon"):
    (
        ...
        phase_velocity_shear,
        material,
    ) = theoretical_velocities(freq, material)
...
def plot_velocities(
    ...
    axs.plot(
        freq, phase_velocities_flexural, label="Simulated velocity", linestyle="--"
    )
    axs.plot(
        freq,
        corrected_phase_velocities,
        label="Simulated corrected velocity",
        linestyle="--",
    )
    ...
    axs.plot(
        freq, wavenumbers_theoretical, label="theoretical wavenumber", linestyle="--"
    )
    axs.set_xlabel("Frequency [kHz]")
...
def plot_velocities_2distance(
    ...
    axs.plot(
        freq1, phase_velocities_flexural, label="Simulated velocity", linestyle="--"
    )
    axs.plot(
        freq1,
        corrected_phase_velocities,
