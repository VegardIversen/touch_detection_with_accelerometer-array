...
def transfer_function():
    SAMPLE_RATE = 150000
    CHANNEL_NAMES_CHIRP = ['Sensor 1', 'Sensor 2', 'Sensor 3', 'Actuator']
    ...
    #filt_touch = signal.filtfilt(b, a, touch_df['Sensor 1'])
    chirp = chirp_df['Actuator']
    chirp_gen = chirp_gen_df['chirp']
    chirp_df_signals = chirp_df.drop(columns=['Sensor 2', 'Actuator'], axis=1)
    chirp_df_signals_cropped = crop_data(chirp_df_signals, 0.5, 3)
    chirp_cropped = crop_data(chirp, 0.5, 3)
    sig_corr1 = signal.correlate(chirp_df_signals['Sensor 1'], chirp, mode='same')
    auto_corr = signal.correlate(chirp, chirp, mode='same')
    sig_corr2 = signal.correlate(chirp_df_signals['Sensor 3'], chirp, mode='same')
    ...
    plt.plot(sig_corr1, label='correlation Sensor 1')
    plt.plot(sig_corr2, label='correlation Sensor 3')
...
# x = np.correlate(chirp_cropped['Sensor 1'], chirp_gen_df['Sensor 1'], 'full')
# ax1 = plt.subplot(211)
# plt.plot(chirp_df['Sensor 1'])
# plt.subplot(212)
# plt.plot(chirp_gen_df['Sensor 1'])
