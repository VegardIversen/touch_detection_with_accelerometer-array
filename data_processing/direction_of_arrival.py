...
def degree_calc(df, upsampl=1):
    ...
    crosscorr2_1 = correlate(df['Sensor 1'], df['Sensor 2'], upSamplingFactor=upsampl) # mic 2 and mic 1
    crosscorr3_1 = correlate(df['Sensor 3'], df['Sensor 2'], upSamplingFactor=upsampl) # mic 3 and 1
    crosscorr3_2 = correlate(df['Sensor 3'], df['Sensor 1'], upSamplingFactor=upsampl) # mic 3 and 2
    autocorr1_1 = correlate(df['Sensor 2'], df['Sensor 2'], upSamplingFactor=upsampl) # mic 1 mic 1, autocorr
...
if __name__ == '__main__':
    ...
    print(test_file)
    df = pd.read_csv(test_file, delimiter=DATA_DELIMITER, names=['Sensor 1', 'Sensor 2', 'Sensor 3'] )
    # print(df.head())
    degree_calc(df)