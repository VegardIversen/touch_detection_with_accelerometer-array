...
def ssqueeze_spectrum(df, channel, sample_rate=150000, focus=True, size=0.75):
    def viz(x, Tx, Wx):
        # plt.imshow(np.abs(Wx), aspect='auto', cmap='turbo')
        fig, axs = figure_size_setup(size)
        axs.imshow(np.abs(Tx), aspect="auto", cmap="viridis")
        plt.show()
        # plt.imshow(np.abs(Tx), aspect='auto', vmin=0, vmax=.2, cmap='turbo')
        fig, axs = figure_size_setup(size)
        axs.imshow(np.abs(Wx), aspect="auto", cmap="viridis")
        axs.set_title("continuous CWT")
        plt.show()
    ...
    if focus:
        fig, axs = figure_size_setup(size)
        data = cut_out_signal(data, 150e3, 0.0022)
        data = data.iloc[:20000]
        print(f"lenght of data: {len(data)}")
        time_axis = np.linspace(0, len(data) // sample_rate, num=len(data))
        print(f"lenght of time axis: {len(time_axis)}")
        axs.plot(time_axis, data)
...
def psudo_wigner_ville_dist(df, channel):
    ...
    pwvd = []
    for seg in segments:
        pwvd_seg = PseudoWignerVilleDistribution(
            seg, window=window_size, n_frequencies=n_frequencies, n_times=n_times
        )
...
def custom_wigner_ville_batch(df, channel, batch_size=100000):
    ...
    time_wvd, freq_wvd = np.meshgrid(
        np.linspace(0, duration, n_segments + 1),
        np.fft.fftfreq(n_fft + 1, 1 / sampling_rate),
    )
...
def plot_2fft(df1, df2, window=False):
    ...
    plt.ylabel("Amplitude")
    plt.plot(
        np.fft.fftshift(fftfreq1), 20 * np.log10(np.abs(np.fft.fftshift(data_fft1)))
    )
    ...
    plt.ylabel("Amplitude")
    plt.plot(
        np.fft.fftshift(fftfreq2), 20 * np.log10(np.abs(np.fft.fftshift(data_fft2)))
    )
    plt.tight_layout()
    plt.show()
...
def plot_spectogram(df, include_signal=True, channel="channel 1", freq_max=None):
...
def compare_signals(
    ...
    plt.subplots_adjust(
        left=0.06, right=0.985, top=0.97, bottom=0.06, hspace=0.3, wspace=0.2
    )
...
def plot_data_vs_noiseavg(df, channel="channel 1"):
    ...
    """
    noise_df = csv_to_df(file_folder="base_data", file_name="df_average_noise")
...
def plot_data_sub_ffts(df, channel="channel 1"):
    ...
    df_sub_noise = pd.DataFrame(
        scipy.fft.ifft(df_fft_sub_noise_fft),
    ...
    fftfreq_data_sub_noise = scipy.fft.fftfreq(
        len(df_fft_sub_noise_fft), 1 / SAMPLE_RATE
    )
        ...
        for file in os.listdir(ROOT_FOLDER + folder):
            if file.endswith(".csv"):
                ...
                if bandwidth is not None:
                    ...
                    files["freq"] = freq
                measurements_filt_comp = compress_chirp(
                    measurements_filt, custom_chirp=None
                )
                if multichannel:
                    print("multichannel")
                    phase1, vph1, _ = get_phase_and_vph_of_compressed_signal(
                        ...
                        distance=np.linalg.norm(
                            SETUP.sensor_1.coordinates - SETUP.sensor_2.coordinates
                        ),
                    ...
                    phase2, vph2, _ = get_phase_and_vph_of_compressed_signal(
                        ...
                        bandwidth=bandwidth,
                        distance=np.linalg.norm(
                            SETUP.sensor_2.coordinates - SETUP.sensor_3.coordinates
                        ),
                        threshold1=threshold1,
                        threshold2=threshold2,
                    )
                    phase3, vph3, _ = get_phase_and_vph_of_compressed_signal(
                        ...
                        bandwidth=bandwidth,
                        distance=np.linalg.norm(
                            SETUP.sensor_1.coordinates - SETUP.sensor_3.coordinates
                        ),
                        threshold1=threshold1,
                        threshold2=threshold2,
                    )
...
def plot_plate_speed_sliders_book():
    ...
    (line,) = ax.plot(
        freqency[freqency > 0],
        f(freqency[freqency > 0], h, init_rho, Poisson, E),
        lw=2,
        label="spoon",
    )
    (line2,) = ax.plot(
        freqency[freqency > 0],
        f(freqency[freqency > 0], h, 550, 0.4, 9.5 * 10**9),
        lw=2,
        label="furugran",
    )
    ...
    rho_slider_spoon = Slider(
        rho_slider_spoon_ax, "rho spoon", rho_start, rho_end, valinit=init_rho
    )
    rho_slider_furugran_ax = fig.add_axes([0.25, 0.1, 0.65, 0.03])

    rho_slider_furugran = Slider(
        rho_slider_furugran_ax, "rho furugran", 400, 700, valinit=550
    )
    # create slider for E value in the range of E start to E end
    E_slider_ax = fig.add_axes([0.25, 0.05, 0.65, 0.03])
    E_slider_furugran = Slider(
        E_slider_ax, "E furugran", 7 * 10**9, 12 * 10**9, valinit=9.5 * 10**9
    )
    ...
    def update(val):
        ...
        line2.set_ydata(
            f(
                freqency[freqency > 0],
                h,
                rho_slider_furugran.val,
                0.4,
                E_slider_furugran.val,
            )
        )
...
def plot_estimated_reflections_with_sliders(setup, measurements_comp):
    ...
    propspeed_slider = Slider(
        propspeed_slider_ax, "propegation speed", 1, 3000, valinit=600
    )
    ...
    def get_arrivl_times(sensors, actuator, prop_speed):
        ...
        arrival_times = np.reshape(
            arrival_times, (len(sensors), len(arrival_times) // len(sensors))
        )
        arrival_times *= 1000  # Convert to ms
        return arrival_times

    time_axis_corr = np.linspace(
        0, 1000 * len(measurements_comp) / SAMPLE_RATE, (len(measurements_comp))
    )
    ...
    for i, sensor in enumerate(sensors):
        ...
        ax[i].plot(
            time_axis_corr,
            measurements_comp["channel " + str(i + 1)],
            label="Correlation",
        )
        ax[i].plot(
            time_axis_corr,
            measurements_comp_hilb["channel " + str(i + 1)],
            label="Hilbert envelope",
        )
        first_arrival = ax[i].axvline(
            arrival_times[i][0], linestyle="--", color="r", label="Direct wave"
        )
        firstref = [
            ax[i].axvline(line, linestyle="--", color="g", label="1st reflections")
            for line in (arrival_times[i][1:5])
        ]
        secondref = [
            ax[i].axvline(line, linestyle="--", color="purple", label="2nd reflections")
...
def inspect_touch(df, savefig=False, file_format="png"):
    ...
    spec[3].set_clim(to_dB(np.max(spec[0])) - 60, to_dB(np.max(spec[0])))
    ...
    for i, channel in enumerate(data):
        ...
        if "time" in plots_to_plot:
            if compressed_chirps:
                ...
                time_axis = np.linspace(
                    start=0, stop=len(channel) / SAMPLE_RATE, num=len(channel)
                )
        ...
        if "spectrogram" in plots_to_plot:
            ...
            if compressed_chirps:
                ...
                #                                   signal_length_seconds))
            spec[3].set_clim(
                to_dB(np.max(spec[0])) - dynamic_range_db, to_dB(np.max(spec[0]))
            )
            if set_index is not None:
                fig.colorbar(
...
def subplots_adjust(signal_type: list, rows: int = 1, columns: int = 1):
    ...
    if (
        ...
        plt.subplots_adjust(
            left=0.18, right=0.971, top=0.927, bottom=0.152, hspace=0.28, wspace=0.2
        )
    elif signal_type == ["time"] and rows == 1 and columns == 1:
        plt.subplots_adjust(
            left=0.12, right=0.98, top=0.9, bottom=0.2, hspace=0.28, wspace=0.2
        )
    elif signal_type == ["time"] and rows == 2 and columns == 1:
        plt.subplots_adjust(
            left=0.153, right=0.98, top=0.957, bottom=0.079, hspace=0.237, wspace=0.2
        )
    elif signal_type == ["time"] and rows == 3 and columns == 1:
        plt.subplots_adjust(
            left=0.125, right=0.965, top=0.955, bottom=0.07, hspace=0.28, wspace=0.2
        )
    elif signal_type == ["spectrogram"] and rows == 1 and columns == 1:
        plt.subplots_adjust(
            left=0.17, right=1, top=0.929, bottom=0.145, hspace=0.28, wspace=0.2
        )
    elif signal_type == ["spectrogram"] and rows == 2 and columns == 1:
        plt.subplots_adjust(
            left=0.167, right=1, top=0.955, bottom=0.08, hspace=0.236, wspace=0.2
        )
    elif signal_type == ["spectrogram"] and rows == 3 and columns == 1:
        plt.subplots_adjust(
            left=0.125, right=1.05, top=0.955, bottom=0.07, hspace=0.28, wspace=0.2
        )
    elif signal_type == ["fft"] and rows == 1 and columns == 1:
        plt.subplots_adjust(
            left=0.121, right=0.98, top=0.926, bottom=0.14, hspace=0.28, wspace=0.15
        )
    elif signal_type == ["fft"] and rows == 2 and columns == 1:
        plt.subplots_adjust(
            left=0.125, right=0.957, top=0.955, bottom=0.075, hspace=0.28, wspace=0.2
        )
    elif signal_type == ["fft"] and rows == 3 and columns == 1:
        plt.subplots_adjust(
            left=0.125, right=0.95, top=0.955, bottom=0.07, hspace=0.28, wspace=0.2
        )
    elif signal_type == ["time", "spectrogram"] and rows == 2 and columns == 1:
        plt.subplots_adjust(
            left=0.18, right=0.97, top=0.955, bottom=0.0, hspace=0.19, wspace=0.2
        )
    elif signal_type == ["setup"]:
        plt.subplots_adjust(
            left=0.13, right=0.97, top=0.97, bottom=0.146, hspace=0.28, wspace=0.2
        )
    else:
...
def figure_size_setup(overleaf_size=0.75):
    sb.set(
        font_scale=12 / 10
    )  # font size = 12pt / 10pt/scale = 1.2 times the default size
...
def figure_size_setup_thesis(overleaf_size=0.75):
    sb.set(
        font_scale=12 / 10
    )  # font size = 12pt / 10pt/scale = 1.2 times the default size
...
def create_figure_thesis(scale=1.0, font_scale=1.2):
    sb.set(font_scale=12 / 10)
    TEXTWIDTH_PT = 369.88583
    inches_per_pt = 1 / 72.27  # Convert pt to inch
    width_in = TEXTWIDTH_PT * inches_per_pt * scale  # width in inches
    height_in = width_in * (2 / 3)  # Golden ratio to set aesthetic figure height

    # Set the font size according to the figure size
