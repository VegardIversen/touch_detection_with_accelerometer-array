...
def compression_figs():
    ...
    fig, axs = figure_size_setup(0.33)
    axs.plot(
        np.fft.rfftfreq(len(signal_compressed), 1 / 1000),
        np.abs(np.fft.rfft(signal_compressed)),
    )
    axs.set_xlabel("Frequency")
    axs.set_ylabel("Amplitude")
    ...
    axs.plot(
        np.fft.rfftfreq(len(signal), 1 / 1000),
        np.abs(np.fft.rfft(signal)) / np.max(np.abs(np.fft.rfft(signal))),
        label="signal",
    )
    axs.plot(
        np.fft.rfftfreq(len(signal_compressed), 1 / 1000),
        np.abs(np.fft.rfft(signal_compressed))
        / np.max(np.abs(np.fft.rfft(signal_compressed))),
        label="compressed",
    )
    axs.set_xlabel("Frequency")
    axs.set_ylabel("Amplitude")
...
def table_figures():
    ...
    SETUP.draw(
        ...
        save_fig=True,
        file_format="svg",
        fig_name="table_setup2_correlation",
        show_tab=True,
    )
    SETUP.draw(
        actuator_show=False,
...
def results_setup1():
    st.transfer_function_plate(
        "\\Measurements\\sensortest\\rot_clock_123",
        n_files=1,
        plot_response=True,
    ...
    st.fft_analysis_one_sens_all_pos(
        folder="\\Measurements\\sensortest\\",
        n_runs=1,
        plot_chirp_fft=True,
        savefig=True,
        file_format="svg",
    ...
    st.fft_analysis_all_sensors(
        "\\Measurements\\sensortest\\",
        position=-3,
        n_runs=1,
        plot_chirp_fft=True,
        savefig=True,
    ...
    st.fft_analysis_all_sensors_compare(
        folder="\\Measurements\\sensortest\\",
        n_runs=1,
        plot_chirp_fft=True,
        savefig=True,
    ...
    st.fft_analysis_all_positions_compare(
        folder="\\Measurements\\sensortest\\rot_clock_123\\",
        savefig=True,
        file_format="png",
    )
...
def results_setup4():
    ...
    phase10, freq10 = wp.phase_plotting(
        df1,
        chirp=custom_chirp,
        ...
        BANDWIDTH=[100, 40000],
        save_fig=False,
        file_name="phase_plot_10cm0_45.svg",
        file_format="svg",
        figsize=0.45,
    )
...
def results_setup5_touch():
    ...
    df_chirp_10cm = csv_to_df(
        "Measurements\\setup3_0\\", "prop_speed_chirp3_setup3_0_v1"
    )
    ...
    measurements_comp, start_indexes = compress_df_touch(
        measurements_filt,
        set_threshold_man=False,
        thresholds=[0.0010, 0.0007, 0.0013],
        n_sampl=max_samples_direct,
    )
    ...
    [
        axs.axvline(line, linestyle="--", color="g", label="1st reflections")
        for line in (arrival_times[0][1:5])
    ]
    [
        axs.axvline(line, linestyle="--", color="purple", label="2nd reflections")
        for line in (arrival_times[0][5:])
    ]
    axs.set_xlabel("Time [s]")
    axs.set_ylabel("Amplitude [V]")
    plot_legend_without_duplicates()
    ...
    fig.savefig(
        f"reflections_speed{np.round(prop_speed, 2)}.png", dpi=300, format="png"
    )
    fig.savefig(
        f"reflections_speed{np.round(prop_speed, 2)}.svg", dpi=300, format="svg"
    )
    # for i, sensor in enumerate(sensors):
...
def results_setup5_chirp():
    ...
    df_chirp_ref_10cm = csv_to_df(
        "Measurements\\setup9_propagation_speed_short\\chirp\\100Hz_to_40kHz_single_chirp\\",
        "chirp_v1",
    )
    custom_chirp = csv_to_df(
        file_folder="Measurements\\div_files",
    ...
    measurements_comp_touch, start_indexes = compress_df_touch(
        df_touch_10cm,
        set_threshold_man=False,
        thresholds=[0.0010, 0.0007, 0.0013],
        n_sampl=max_samples_direct,
    ...
    wrong_start = get_first_index_above_threshold(
        measurements_comp_touch["channel 1"], 0.0001
    )
    ...
    axs.plot(
        time_axis,
        measurements_compressed_hilbert["channel 1"],
    ...
    fig.savefig(
        f"chirp_reflections_speed{np.round(prop_speed, 2)}.png", dpi=300, format="png"
    )
    fig.savefig(
        f"chirp_reflections_speed{np.round(prop_speed, 2)}.svg", dpi=300, format="svg"
    )
    plt.show()
    plt.clf()
    fig, axs = figure_size_setup()
    ...
    fig.savefig(
        f"touch_reflections_speed{np.round(prop_speed, 2)}.png", dpi=300, format="png"
    )
    fig.savefig(
        f"touch_reflections_speed{np.round(prop_speed, 2)}.svg", dpi=300, format="svg"
    )
    plt.show()
