...
def setup1_plot_touch_signals():
    ...
    for channel in CHANNELS_TO_PLOT:
        setup1_plot_touch_spectrogram(
            measurements_full_touch, [channel], dynamic_range_db=60
        )
        channel_file_name = channel.replace(" ", "").lower()
    ...
    measurements_beginning_of_touch = crop_data(
        measurements_full_touch, time_start=(2.05 + 0.0538), time_end=(2.05 + 0.07836)
    )
    for channel in CHANNELS_TO_PLOT:
    ...
    for channel in CHANNELS_TO_PLOT:
        setup1_plot_touch_spectrogram(
            measurements_beginning_of_touch, [channel], dynamic_range_db=60, nfft=1024
        )
        channel_file_name = channel.replace(" ", "").lower()
        file_name = f"setup1_touch_spectrogram_{channel_file_name}_beginning.pdf"
        plt.savefig(FIGURES_SAVE_PATH + file_name, format="pdf")

    fig, axs = plt.subplots(nrows=1, ncols=1, figsize=set_window_size(), squeeze=False)

    CHANNELS_TO_PLOT = ["Sensor 1", "Sensor 3"]
    for channel in CHANNELS_TO_PLOT:
        setup1_plot_touch_fft(measurements_beginning_of_touch, fig, axs, [channel])
...
def setup1_plot_chirp_signals():
    ...
    for channel in CHANNELS_TO_PLOT:
        setup1_plot_chirp_spectrogram(measurements_raw, [channel], dynamic_range_db=60)
        channel_file_name = channel.replace(" ", "").lower()
    ...
    fig, axs = plt.subplots(nrows=1, ncols=1, figsize=set_window_size(), squeeze=False)
    for channel in CHANNELS_TO_PLOT:
        setup1_plot_chirp_fft(measurements_raw, fig, axs, [channel])
        channel_file_name = channel.replace(" ", "").lower()
    ...
    measurements_compressed = measurements_compressed.loc[
    ...
    for channel in CHANNELS_TO_PLOT:
        setup1_plot_chirp_spectrogram(
            measurements_compressed, [channel], dynamic_range_db=60, nfft=512
        )
        channel_file_name = channel.replace(" ", "").lower()
    ...
    fig, axs = plt.subplots(nrows=1, ncols=1, figsize=set_window_size(), squeeze=False)
    for channel in CHANNELS_TO_PLOT:
        setup1_plot_chirp_fft(measurements_compressed, fig, axs, [channel])
        channel_file_name = channel.replace(" ", "").lower()
    axs[0, 0].set_ylim([-20, 100])
    axs[0, 0].legend(CHANNELS_TO_PLOT)
    # axs[0, 0].set_title('')
...
def setup1_plot_touch_time_signal(
    measurements: pd.DataFrame,
    channels_to_plot: list = ["Actuator", "Sensor 1", "Sensor 3"],
):
    ...
    fig, axs = plt.subplots(
        ...
        figsize=set_window_size(rows=len(channels_to_plot), cols=len(PLOTS_TO_PLOT)),
        squeeze=False,
    )
    compare_signals(
        fig,
...
def setup1_plot_touch_spectrogram(
    measurements: pd.DataFrame,
    channels_to_plot: list = ["Sensor 1", "Sensor 3"],
...
def setup1_plot_touch_fft(
    measurements: pd.DataFrame, fig, axs, channels_to_plot=["Sensor 1", "Sensor 3"]
):
...
def setup1_plot_chirp_time_signal(
    measurements: pd.DataFrame,
    channels_to_plot: list = ["Actuator", "Sensor 1", "Sensor 3"],
    envelope: bool = False,
):
    ...
    if envelope:
        compare_signals(
            fig,
            axs,
            [get_envelopes(measurements[channel]) for channel in channels_to_plot],
            plots_to_plot=PLOTS_TO_PLOT,
            compressed_chirps=False,
        )
        """Re-add grid"""
...
def setup1_plot_chirp_spectrogram(
    measurements: pd.DataFrame,
    channels_to_plot: list = ["Actuator", "Sensor 1", "Sensor 3"],
    dynamic_range_db: int = 60,
    nfft: int = 4096,
...
def setup1_transfer_function(setup: Setup):
    ...
    measurements["Sensor 1"] = window_signals(
        pd.DataFrame(measurements["Sensor 1"]),
        direct_signal_seconds_sensor1,
        window_function="tukey",
        window_parameter=0.1,
    )
    direct_signal_seconds_sensor3 = 0.00085
    measurements["Sensor 3"] = window_signals(
        pd.DataFrame(measurements["Sensor 3"]),
    ...
    fft_frequencies = scipy.fft.fftfreq(fft_input.size, d=1 / SAMPLE_RATE)
    ...
    transfer_function = transfer_function[
        (fft_frequencies >= 1) & (fft_frequencies <= 40000)
    ]
    ...
    distance_between_sensors = np.linalg.norm(
        setup.sensors[SENSOR_3].coordinates - setup.sensors[SENSOR_1].coordinates
    )
    phase_velocities = -(
        2 * np.pi * fft_frequencies * distance_between_sensors / phase_response
    )
    ...
    ) = simulated_phase_velocities(fft_frequencies)

    """Plot the phase velocities"""
    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=set_window_size(), sharex=True)
    ax.plot(
        fft_frequencies / 1000,
        simulated_phase_velocities_uncorrected,
    ...
    plt.savefig(FIGURES_SAVE_PATH + FILE_NAME, format="pdf")
    ax.plot(fft_frequencies / 1000, phase_velocities, label="Measured velocities")
...
def setup1_scattering():
    ...
    axs[0, 0].set_ylabel("Acceleration [$\mathregular{m/s^2}$]")
    ...
    for channel in measurements:
        measurements[channel] = np.roll(measurements[channel], -SHIFT_BY)
    ...
    reference_chirp = np.pad(chirp, (PRE_PAD_LENGTH, POST_PAD_LENGTH), mode="constant")
    _, ax = plt.subplots(1, 1)
    ax.plot(np.linspace(-2.5, 2.5, measurements.shape[0]), reference_chirp)
    for channel in measurements:
        measurements[channel] = signal.correlate(
            measurements[channel], reference_chirp, mode="same"
        )
    ...
    measurements_split = correct_drift(
        measurements_split, data_to_sync_with=measurements_split
    )
    ...
    for channel in CHANNELS_TO_PLOT:
        ...
        ax.grid()
        plt.subplots_adjust(
            left=0.19, right=0.967, top=0.921, bottom=0.155, hspace=0.28, wspace=0.2
        )
        channel_file_name = channel.replace(" ", "").lower()
...
def setup1_predict_reflections_in_envelopes(setup: Setup, measurements: pd.DataFrame):
    ...
    envelope_with_lines(setup.sensors[SENSOR_1], measurements, arrival_times)
    adjust_plot_margins()
    ...
    correction_offset_time = 2 * correction_offset / SAMPLE_RATE
    arrival_times += correction_offset_time
    envelope_with_lines(setup.sensors[SENSOR_3], measurements, arrival_times)
    adjust_plot_margins()
...
def setup1_predict_reflections_in_spectrograms(
    setup: Setup, measurements: pd.DataFrame, nfft=1024, dynamic_range_db=60
):
    propagation_speed = setup.get_propagation_speed(measurements, prominence=0.2)
    """Calculate arrival times for sensor 1"""
    arrival_times, _ = get_travel_times(
        setup.actuators[ACTUATOR_1],
        setup.sensors[SENSOR_1],
    ...
    """Plot lines for expected arrival times"""
    spectrogram_with_lines(
        setup.sensors[SENSOR_1], measurements, arrival_times, nfft, dynamic_range_db
    )
    adjust_plot_margins()
    plt.savefig(
        FIGURES_SAVE_PATH + "predicted_arrivals_spectrogram_sensor1_setup1.pdf",
        format="pdf",
    )
    ...
    arrival_times += 2.5
    spectrogram_with_lines(
        setup.sensors[SENSOR_3], measurements, arrival_times, nfft, dynamic_range_db
    )
    adjust_plot_margins()
    plt.savefig(
        FIGURES_SAVE_PATH + "predicted_arrivals_spectrogram_sensor3_setup1.pdf",
        format="pdf",
    )
...
def setup2_results():
    ...
    SETUP.draw()
    adjust_plot_margins()
    plt.savefig(FIGURES_SAVE_PATH + "setup2_draw.pdf", format="pdf")

    setup2_transfer_function(SETUP)


def setup2_plot_time_signals(
    measurements: pd.DataFrame,
    ...
    fig, axs = plt.subplots(
        ...
        ncols=len(plots_to_plot),
        figsize=set_window_size(rows=measurements.shape[1], cols=len(plots_to_plot)),
        squeeze=False,
    )
    compare_signals(
        fig,
...
def setup2_plot_spectrogram_signals(
    measurements: pd.DataFrame,
    signal_start_seconds: float,
    signal_length_seconds: float,
):
...
def find_and_plot_power_loss(
    ...
    PLOTS_TO_PLOT = ["fft"]
    fig, axs = plt.subplots(
        nrows=1, ncols=len(PLOTS_TO_PLOT), figsize=set_window_size(), squeeze=False
    )
    compare_signals(
        fig,
        axs,
...
def setup3_results():
    ...
    SETUP.draw()
    adjust_plot_margins()
    plt.savefig(FIGURES_SAVE_PATH + "setup3_draw.pdf", format="pdf")
...
def setup3_plot_raw_time_signal():
    ...
    """Plot raw signal"""
    time_axis = np.linspace(
        start=0, stop=1000 * len(measurements) / SAMPLE_RATE, num=len(measurements)
    )
    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=set_window_size())
...
def scattering_figure_3():
    ...
    ax.plot(ka_1.theta, ka_1.r, label="Rigid inclusion")
    ax.legend()
    ax.grid()
...
def scattering_figure_5():
    ...
    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=set_window_size())
    ax.plot(
        bonded_steel_inclusion_thickness_0_5in.x,
        bonded_steel_inclusion_thickness_0_5in.y,
        label="Bonded steel inclusion, thickness 0.5 in",
    )
    ax.plot(
        steel_thickness_50in.x,
        steel_thickness_50in.y,
        label="Heavy, bonded steel inclusion, thickness 50 in",
    )
    ax.plot(hole.x, hole.y, label="Limiting case of a hole")
    ax.plot(rigid_inlcusion.x, rigid_inlcusion.y, label="Rigid inclusion")
    ax.set_xlabel("ka")
