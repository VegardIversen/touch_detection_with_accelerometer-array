...
def linear_array_by_edge():
    ...
    measurements["Sensor 1"], measurements["Sensor 3"] = (
        measurements["Sensor 3"],
        measurements["Sensor 1"].copy(),
    )
...
def calculate_source_location(measurements, distances, setup):
    ...
    for i, channel in enumerate(["Sensor 1", "Sensor 2"]):
        ...
        sorted_peaks = sorted(
            zip(peak_indices, peak_heights), key=lambda x: x[1], reverse=True
        )
        ...
        for peak in highest_peaks:
            ...
            ax.axvline(x=peak_x, color=color, linestyle="--", label=f"{channel} peaks")
    plot_legend_without_duplicates(placement="upper right")
    ax.grid()

    peaks_indices = peaks_indices / SAMPLE_RATE
    propagation_speed_sensor1 = np.abs(
    ...
    propagation_speed_sensor2 = np.abs(
        (distances[1][1] - distances[1][0])
        / ((peaks_indices[1][0] - peaks_indices[1][1]))
    )
    print(f"Propagation speed sensor 2: {propagation_speed_sensor2:.2f} m/s")
    propagation_speed_mean = (propagation_speed_sensor1 + propagation_speed_sensor2) / 2
    ...
    distance_to_source = np.abs(
        (2 * 0.05) / ((np.tan(theta_rad) - np.tan(phi_rad)) * np.cos(phi_rad))
    )
    print(f"Distance to source: {distance_to_source:.2f} m")
    plt.figure()
    setup.draw()
    estimated_source_x = (
        setup.sensors[SENSOR_1].x + setup.sensors[SENSOR_2].x
    ) / 2 - distance_to_source * np.cos(phi_rad)
    estimated_source_y = (
        setup.sensors[SENSOR_1].y + setup.sensors[SENSOR_2].y
    ) / 2 - distance_to_source * np.sin(phi_rad)
    plt.scatter(
        estimated_source_x,
        estimated_source_y,
